...
const SuggestionsSection: React.FC<SuggestionsSectionProps> = ({
  ...
  <div className="mt-12 mx-4 my-8">
      ...
      <>
        <div className="flex justify-center items-center mb-4">
          <ArrowRightCircleIcon
            className="animate-spin h-5 w-5 mr-3 text-gray-300"
            aria-hidden="true"
          />
    ...
    {suggestions && suggestions.length > 0 && (
      <SuggestionsGrid suggestions={suggestions} sendMessage={sendMessage} />
    )}
  </div>
);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
      {suggestions.map((suggestion, index) => (
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            handleButtonClick(suggestion.title, suggestion.content);
          }}
        >
          <div className="border rounded-xl p-2 border-gray-500 hover:border-gray-100">
            <button
              type="submit"
              className="btn relative btn-neutral group w-full rounded-xl text-left shadow-md text-gray-300"
            >
              <div className="p-2">
