...
function ThreadRow({id, title, onDelete}: Props) {
  ...
  return (
    <Link href={`/thread/${id}`} className={`rounded-lg px-5 py-3 text-sm items-center
    ...
    "bg-gray-700/50" }`}>
        ...
        <TrashIcon onClick={removeThread} className='h-5 w-5 text-gray-700 hover:text-red-700'/>
    </Link>
  )
}
